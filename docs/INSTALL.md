...
Товардын сүрөттөрү (§12-Б.1) `product_images` таблицасында, байттары менен
базанын өзүндө сакталат. Демек `pg_dump` аларды кошо алат — өзүнчө папканын
көчүрмөсү керек эмес, база менен сүрөттөр эч качан ажырап калбайт.

> Ошондуктан dump файлы сүрөттөрдүн эсебинен чоңоёт. Ар бир сүрөт серверде
> 1400px'ке чейин кичирейтилип, JPEG кылып сакталат (болжол менен 200–400 КБ),
> бир товарга 8ден көп эмес.
